...
function uploadSnapshot() {
    ...
    $.ajax({
        url: EXPERIMENTATION_SERVER_URL + '/snapshot/upload',
        type: 'POST',
        data: { user_id: id_calibrated_model, image_blob: current_frame },
        success: function(result) {
    ...
    // Notify user of the process
    console.log('Snapshot taken and uploaded.');
    window.alert('Snapshot taken and uploaded.');
}
...
function trainSnapshot() {
    ...
    $.ajax({
        ...
        success: function(result) {
            console.log('Training initiated.');
            window.alert('A training procedure has been initiated based on the snapshot. You can close this window at any time.');
        },
        error: function(error) {
            console.log(`Error in training call: ${error}`);
            window.alert('Could not call for a training procedure at this time. Please try again later.');

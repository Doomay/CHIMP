...
from logic.emotionrecognition.data import NewEmotionCalibrationDataProcessor
...
def build_emotion_recognition_pipeline(config: dict, do_calibrate_base_model: bool = False):
    ...
    if config['use_mlflow']:
        ...
        if do_calibrate_base_model:
            if not config['retrain_with_old_data']:
                return MLFlowPipeline(config=config, data_processor=NewEmotionCalibrationDataProcessor,

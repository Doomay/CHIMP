...
from logic.emotionrecognition.__utilities import save_data_object, split_data, fetch_models
    ...
    def _publish_models(self):
        ...
        if path.exists(self._config["publish_directory"]):
            ...
            for i in range(len(best_models)):
                ...
                log = log_model(onnx_model=onnx_model, artifact_path="model",
                          registered_model_name=self._config['model_name'])

                #best_models_list.append(log.run_id)

        # Save the RUN ID of the actual best
        #input_sig = [tf.TensorSpec([None, self._config['image_height'], self._config['image_width'], 1], tf.float32)]
        #best_model = best_n_function(len(best_models), self.models, key=lambda model:model[evaluation_metric])
        #onnx_model, _ = tf2onnx.convert.from_keras(best_model[0]['model'], input_sig, opset=13)
        #log = log_model(onnx_model=onnx_model, artifact_path="model",
                        #registered_model_name=self._config['model_name'])
        #fetch_models(log.run_id)
        return best_models
...
class MLFlowEmotionModelPublisher(EmotionModelPublisher):
    ...
    def _publish_models(self):
        ...
        eval_metric = self._config['best_model_publish_metric']
        fetch_models(model_run_id, best_model_entry[eval_metric])
        return best_models
...
class EmotionCalibrationModelPublisher(ModelPublisherABC):
    ...
    def _publish_models(self):
        ...
        input_sig = [tf.TensorSpec([None, self._config['image_height'], self._config['image_width'], 1], tf.float32)]
        onnx_model, _ = tf2onnx.convert.from_keras(model, input_sig, opset=13)
        new_model = log_model(onnx_model=onnx_model, artifact_path='model', registered_model_name=self._config['model_name'])

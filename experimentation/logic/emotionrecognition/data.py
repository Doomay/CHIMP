...
class NewEmotionCalibrationDataProcessor(DataProcessorABC):
    """A custom data processing unit for emotion recognition data for model calibration.
    ...
    def _load_data(self):
        ...
        # Define a function to add new data items to the data dictionary
        def add_data_item(image_data_, class__, category_):
            data['image_data'].append(image_data_)
            data['class_'].append(class__)
            data['category'].append(category_)
        ...
        def create_dataset(calibration_data: dict = None, split: str = 'training'):
            if calibration_data is None:
                dataset = {
                    'image_data': [],
                    'class_': [],
             ...
             # Read from config   > pd
            if split == 'training':
                dataset['image_data'] = data['image_data'][:int(0.8 * len(data['image_data']))]
                dataset['class_'] = data['class_'][:int(0.8 * len(data['class_']))]
                dataset['category'] = data['category'][:int(0.8 * len(data['category']))]
            elif split == 'validation':
                dataset['image_data'] = data['image_data'][int(0.8 * len(data['image_data'])):int(0.9 * len(data['image_data']))]
                dataset['class_'] = data['class_'][int(0.8 * len(data['class_'])):int(0.9 * len(data['class_']))]
                dataset['category'] = data['category'][int(0.8 * len(data['category'])):int(0.9 * len(data['category']))]
            elif split == 'test':
                dataset['image_data'] = data['image_data'][int(0.9 * len(data['image_data'])):]
                dataset['class_'] = data['class_'][int(0.9 * len(data['class_'])):]
                dataset['category'] = data['category'][int(0.9 * len(data['category'])):]
            else:
                raise ValueError("Invalid split parameter. Use 'training', 'validation', or 'test'.")
        ...
        training_data = create_dataset(calibration_data=data, split='training')
        validation_data = create_dataset(calibration_data=data, split='validation')
        test_data = create_dataset(calibration_data=data, split = 'test')
    ...
    def _process_data(self):
        ...
        # Reshape data (greyscale shape), transform data into numpy arrays for the neural network
        data = self.data
        reshaped_data = np.array(data['image_data']) \
            .reshape((-1, self._config['image_height'], self._config['image_width'], 1))
